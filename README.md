...
- 当前本地候选：`0.9.0+87`（`versionCode=87`）；Release/Debug 产物位于工作区根目录，线上 GitHub Release 尚未在本轮上传。
- 更新包支持 GitHub API 有界重试、Range 续传、真实字节/速度进度、SHA-256 校验和用户确认安装；发布 CI 会校验 Tag、包信息、签名和 manifest。
- 网络层提供来源健康冷却、可信 DNS 短时缓存和已验证来源能力矩阵；家庭 Wi‑Fi 真实回退仍需现场验证。
- 设置页支持书架 JSON 备份导入/导出；章节文件原子写入；阅读器字号/行距/夜间设置可持久化。
